...
export default function AdminStoresPage() {
  ...
  const fetchStores = useCallback(async () => {
    ...
    try {
      const { data } = await api.get("/stores?" + query.join("&"));
      setStores(data);
    } catch {
      setErr("Could not fetch stores");
    }
  }, [search, sort]);
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setErr(e.response?.data?.error || "Could not create store");
    }
  };
  ...
  return (
    <Container>
      <Card>
        <h2 style={{ color: theme.primary, marginBottom: "1.2rem" }}>Stores</h2>
          ...
          <tbody>
            {stores.map(store => (
              <tr key={store.id}>
                ...
                <TD>{store.email}</TD>
                <TD>{store.address}</TD>
                <TD>{store.rating ? store.rating.toFixed(2) : "N/A"}</TD>
              </tr>
            ))}
          </tbody>
        </Table>
      ...
      <Card>
        <h3 style={{ color: theme.primary, marginBottom: "1.1rem" }}>Add New Store</h3>
        {err && <Error>{err}</Error>}
